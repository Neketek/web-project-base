...
def create(self,data):
    ...
    try:
        email_entity = self.sql_session.query(sql.Email).filter(sql.Email.email==email).one()
        try:
            self.sql_session.query(sql.User).filter(sql.User.email==email_entity).one()
            raise UserFriendlyException(message='Email is already registered')
    ...
    with sql_session.no_autoflush:
        ...
        password_hash,password_salt = sql.User.hash_user_password(password)
        user_entity.password_hash = password_hash
        user_entity.password_salt = password_salt
        user_entity.email_verification_token = UserEmailVerificationToken()
        user_entity.phone_verification_token = UserPhoneVerificationToken()
